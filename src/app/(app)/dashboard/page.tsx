...
export default async function Dashboard({ searchParams }: { searchParams: Promise<Filters> }) {
  ...
  return <>
    ...
    <section className="dashboard-overview"><OverviewCard href={`/dashboard/detalle/eventos?${toQuery({ ...filters, from, to })}`} label="Eventos del período" value={String(events.length)} detail="Ver detalle" /><MoneyCard href={`/dashboard/detalle/cobrado?${toQuery({ ...filters, from, to })}`} label="Cobrado" totals={collected} currencies={currencies} detail="Ver detalle" /><MoneyCard href={`/dashboard/detalle/a-cobrar?${toQuery({ ...filters, from, to })}`} label="A cobrar" totals={receivable} currencies={currencies} detail="Ver detalle" /><MoneyCard href={`/dashboard/detalle/costos?${toQuery({ ...filters, from, to })}`} label="Costos" totals={costs} currencies={currencies} detail="Ver detalle" /><MoneyCard href={`/dashboard/detalle/ganancia?${toQuery({ ...filters, from, to })}`} label="Ganancia" totals={profit} currencies={currencies} detail="Ver detalle" tone="result" /></section>
  </>;
}
