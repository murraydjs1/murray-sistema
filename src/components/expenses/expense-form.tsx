...
export function ExpenseForm({eventId,eventDate,categories,staff,quoteItems,accounts,suggestedPaidByStaffId}:{eventId:string;eventDate:string;categories:Option[];staff:Option[];quoteItems:Option[];accounts:Array<Option&{currency:string}>;suggestedPaidByStaffId?:string|null}){
 ...
 return <form action={createExpense.bind(null,eventId)} className="form expense-form">
  ...
  <div className="form-grid">
   ...
   <div className="field"><label htmlFor="expense-account">Cuenta Murray (si pagó Murray)</label><select id="expense-account" name="accountId" defaultValue=""><option value="">No pagó una cuenta Murray</option>{accounts.map(a=><option key={a.id} value={a.id}>{a.name} · {a.currency}</option>)}</select></div>
