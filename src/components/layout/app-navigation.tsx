...
import { BarChart3, CalendarDays, Contact, FileText, Landmark, LayoutDashboard, LogOut, Menu, Music2, PackagePlus, ReceiptText, Upload, UserRoundCog, Users } from "lucide-react";
...
export function AppNavigation({role,name,logoutAction}:{role:Role;name:string;logoutAction:()=>Promise<void>}){
 ...
 const primary=management.filter(i=>["/dashboard","/agenda","/presupuestos","/clientes"].includes(i.href));const more=management.filter(i=>["/personal","/catalogo","/gastos","/tesoreria","/reportes/rentabilidad","/configuracion/importar-excel"].includes(i.href));if(role==="ADMIN")more.push({href:"/usuarios",label:"Usuarios",icon:Users});
