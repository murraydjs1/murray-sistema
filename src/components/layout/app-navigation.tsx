...
function NavLink({ item, compact = false }: { item: NavItem; compact?: boolean }) {
  ...
  return (
    <Link href={item.href} className={selected ? "active" : ""} aria-current={selected ? "page" : undefined}>
      <Icon size={compact ? 21 : 19} strokeWidth={1.8} />
...
export function AppNavigation({ role, name, logoutAction }: { role: Role; name: string; logoutAction: () => Promise<void> }) {
  ...
  const desktop = staff
    ? [{ href: "/staff", label: "Mis eventos", icon: CalendarDays }]
    : role === "ADMIN"
      ? [...management, { href: "/usuarios", label: "Usuarios", icon: Users }]
      : management;
  const primary = management.filter((i) => ["/dashboard", "/agenda", "/eventos", "/clientes"].includes(i.href));
  const more = management.filter((i) => ["/personal", "/catalogo", "/gastos", "/tesoreria", "/reportes/rentabilidad", "/configuracion/importar-excel"].includes(i.href));
  if (role === "ADMIN") {
    primary.push({ href: "/usuarios", label: "Usuarios", icon: Users });
    more.push({ href: "/usuarios", label: "Usuarios", icon: Users });
  }
