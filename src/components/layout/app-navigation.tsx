...
import { BarChart3, CalendarDays, Contact, FileText, LayoutDashboard, LogOut, Menu, Music2, PackagePlus, ReceiptText, UserRoundCog, Users } from "lucide-react";
...
const management: NavItem[] = [
  ...
  {href:"/gastos",label:"Gastos",icon:ReceiptText},{href:"/reportes/rentabilidad",label:"Rentabilidad",icon:BarChart3},
];
...
export function AppNavigation({role,name,logoutAction}:{role:Role;name:string;logoutAction:()=>Promise<void>}){
 ...
 const primary=management.filter(i=>["/dashboard","/agenda","/presupuestos","/clientes"].includes(i.href));const more=management.filter(i=>["/personal","/catalogo","/gastos","/reportes/rentabilidad"].includes(i.href));if(role==="ADMIN")more.push({href:"/usuarios",label:"Usuarios",icon:Users});
