...
export function StatusBadge({status}:{status:string}){const tone=status==="REALIZADO"?"success":status==="PENDIENTE"||status.includes("ESPERANDO")?"warning":status==="CANCELADO"?"danger":status==="CONFIRMADO"?"brand":"neutral";return <Badge tone={tone}>{status.replaceAll("_"," ")}</Badge>}
