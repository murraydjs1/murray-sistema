...
describe("role permissions", () => {
  it("only ADMIN can manage users", () => {
    ...
    expect(hasPermission(UserRole.OPERACIONES,"MANAGE_USERS")).toBe(false);
  ...
  it("ADMIN and ADMIN_FINANCIERO can manage Sprint 1 commercial data", () => {
    ...
    expect(hasPermission(UserRole.OPERACIONES,"MANAGE_COMMERCIAL")).toBe(true);
  ...
  it("only management roles can manage staff finances", () => {
    ...
    expect(hasPermission(UserRole.OPERACIONES,"MANAGE_STAFF")).toBe(false);
    expect(hasPermission(UserRole.STAFF,"MANAGE_STAFF")).toBe(false);
  });
  it("OPERACIONES can coordinate events without administrative privileges", () => {
    expect(hasPermission(UserRole.OPERACIONES,"MANAGE_OPERATIONS")).toBe(true);
    expect(hasPermission(UserRole.OPERACIONES,"MANAGE_USERS")).toBe(false);
  });
});
